...
| FB-5 | P2 | Channel list: Get List + min-users filter | C++ has a Get List button (fires /LIST from dialog), min-users spinbox, and search-clear. Python channel list dialog likely lacks all three. See CHANNEL_LIST_DESIGN.md for design + batching notes. | OPEN |
| FB-6 | P3 | Per-channel comic view toggle | C++ (2026-06-11): Ctrl+M / Comic button toggles panel view per channel buffer; backend keeps running globally. Python's `setComicMode` is always global — pressing the button on one channel hides panels everywhere. Python change: add `_comic_hidden_channels: set` (keyed on (network, target)); toggle it on Ctrl+M / action; apply in `_switch_to`. | OPEN |
...
| BP-12 | P2 | Key-redirect / input focus grab | Python has `app.installEventFilter(self)` + `_redirect_key` (typing anywhere jumps to message box, with 6 guards). C++ now has it too (2026-06-11, INPUT_FOCUS_DESIGN.md). **The Python version is already correct — no backport needed.** Noted here for parity record. | DONE (C++ only) |
...
| 31 | user | MISS | Scripts tab load/unload | Preferences → Scripts only shows currently-loaded scripts. Needs a proper manager: list all scripts in the scripts dir (loaded + unloaded), load/unload buttons, reload button, status indicator per script. Mirror Python's scripts dialog. C++ scripting engine is complete; this is purely the Preferences UI surface. | OPEN |
...
| 32 | review | BUG | QMenu::Hide focus-restore fires for all context menus | `eventFilter` hooked `QEvent::Hide` on every `QMenu` and deferred `m_input->setFocus()`. Any right-click context menu close stole focus from the network tree / member list. Fix: guard with `qobject_cast<QMenuBar*>(menu->parentWidget()) != nullptr` so only menu-bar dropdowns trigger the restore; add `activePopupWidget` guard so sub-menu close with parent still open doesn't steal focus. | **DONE** (d14a2d7) |
| 33 | review | BUG | `setComicMode` set `m_comicMode=true` on first enable but never set it back to `false` | Once comic was enabled globally there was no code path that cleared `m_comicMode`. Ctrl+M on a channel hid the panel for that channel only; the global backend ran forever. Fix: pressing Ctrl+M on the **server buffer** now acts as the global off — sets `m_comicMode=false`, clears `m_comicHiddenBuffers`, hides the view. Server-buffer enable still shows "not available" (enable from a channel). | **DONE** (d14a2d7) |
| 34 | review | BUG | `redirectKeyToInput` returned `true` (consumed keystroke) when `m_input` was `nullptr` | The `return true` sat outside the `if (m_input != nullptr)` block, silently eating keystrokes during startup/teardown when the input widget hadn't been created or was already deleted. Fix: moved `return true` inside the guard; added `return false` for the null case. | **DONE** (d14a2d7) |
| 35 | review | BUG | Buffer tab "Close" lambda captured stale `int index` | The close lambda used `[this, index](){closeBufferTab(index);}`. If a channel joined or parted during `QMenu::exec()` (which spins a nested event loop), `syncBufferTabs` ran and shifted tab indices. The lambda then closed the wrong tab. Fix: capture `closeTarget`/`closeNetwork` strings; re-derive the index at close time by scanning `treeItemForTabIndex`. | **DONE** (d14a2d7) |
| 36 | review | BUG | `resetAllSettings` left all window geometry intact | `resetAllSettings` called `saveRaw(defaultSettings())`. `saveRaw` was designed to preserve `geom_*` keys so partial saves don't wipe positions — but that also meant a full reset silently kept every dialog and window size/position. Fix: added `bool preserveGeometry = true` param to `saveRaw`; `resetAllSettings` passes `false`. | **DONE** (d14a2d7) |
| 37 | review | PERF | `syncBufferTabs` ran when `setBufferTabsVisible(false)` was called | Hiding the tab bar still rebuilt every tab entry (O(open channels)). Wasted work since the tab bar is invisible and no code reads from it while hidden. Fix: guarded `syncBufferTabs()` with `if (visible)` in `setBufferTabsVisible`. | **DONE** (d14a2d7) |
