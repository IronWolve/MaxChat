...
**Last session:** 2026-06-10 (post-audit backlog grind) — Scripting switched to **Lua** (API + 3 example scripts converted, SCRIPTING.md; engine #6 still to build). Cleared backlog #2/#3/#4/#5/#8/#10/#11/#17/#26. 42/42 green. Remaining: comic #12/#14/#15/#16/#18, sounds #21/#22, update checker #20, Lua engine #6, tests #7/#13/#19, Python backports.
...
| 20 | 7 (S2) | MISS | Update checker | Help ▸ Check for Updates + optional quiet startup check (pref `update_check`). HTTPS GitHub releases API (IronWolve/MaxChat); manual + startup (pref, default off pre-release). | **DONE** |
...
| 27 | media | MISS | Image paste → upload → inline link | C++ ignores clipboard image paste (the `on_image_paste` script hook is also unported, S13). Python keeps this OUT of core as an opt-in plugin (`scripts-examples/_image_paste_upload.py`, host TBD — uguu.se works; catbox/litterbox are Cloudflare-blocked from Qt's stack). For C++: intercept image paste in the input (`QGuiApplication::clipboard()->image()`), POST to a configurable host, then insert the returned URL (the new inline-image loader will then preview it). Decide/confirm the default host before shipping. | OPEN (TBD) |
