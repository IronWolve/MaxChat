...
| 14 | 6 (S10a) | MISS | Comic per-channel overrides | Implement `comic_channels` `{"<net>/<#chan>": {bg, chars{nick:stem}, ignore[]}}`: a Channels page in ComicSettingsDialog + lookups in refreshComic/comicBackground/comicCharacterForNick that prefer per-channel over global. Mirror Python comic_settings.py. | **DONE** (389d0d4 engine; editor UI = #14b) |
